...
class ParametresSynthese:
    ...
    Ici, n_periodes indique le nombre de jours à remonter depuis "maintenant".
    """
    ...
    def __init__(self, config_path: str = DEFAULT_CONFIG_PATH):
        ...
        self.n_periodes = config["n_periodes"]  # Nombre de jours avant maintenant

        # Coefficients pour le modèle (générés de manière stochastique)
        self.beta_prix = -abs(
...
class GenerateurDonnees:
    ...
    def generer_produits(self) -> pd.DataFrame:
        ...
        for idx, categorie_nom in enumerate(categories_list):
            ...
            for sku_id in range(1, skus_par_categorie[idx] + 1):
                ...
                # Stocker le pourcentage de plancher
    ...
    def generer_dates(self) -> pd.DataFrame:
        ...
        # Optionnel : si vous voulez affiner la journée en cours (e.g. toutes les heures jusqu'à now),
    ...
    def ajuster_prix_et_calculs(self) -> pd.DataFrame:
        ...
        """
        prix_liste = []
        # On fixe un point de repère (pour l'AgeProduitEnJours, par ex.)
...
def main(output_path="Data/donnees_synthetiques.csv"):
    ...
    # 1. Chargement des paramètres par défaut (config.yaml local)
    ...
    # 3. Génération finale du DataFrame
